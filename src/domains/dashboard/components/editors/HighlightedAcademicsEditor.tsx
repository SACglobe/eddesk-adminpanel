...
import { Check, X, Plus, Trash2, LayoutGrid, Star } from "lucide-react";
...
export default function HighlightedAcademicsEditor({ component, schoolKey }: HighlightedAcademicsEditorProps) {
    ...
    const handleSelectRecordFromIndex = async (slotIndex: number, recordKey: string) => {
        ...
        try {
            ...
            const response = await upsertComponentData('componentplacement', {
                ...
                componentcode: component.componentcode || 'highlightedacademics',
    ...
    const handleSave = async () => {
        if (!editingItem.title?.trim()) {
            alert("Please enter a featured title.");
            return;
        }
        if (!editingItem.subtitle?.trim()) {
            alert("Please enter a subtitle or grade level.");
            return;
        }
        ...
        try {
            ...
            if (pendingFile) {
                setIsUploading(true);
                try {
                    const uploadedUrl = await uploadFile(pendingFile, schoolKey, "academics");
    ...
    const slots = useMemo(() => {
        ...
        if (config?.selectionmethod === "auto") {
            // In auto mode, we just show the first record that matches our filters
    ...
    return (
        <BaseEditor
            title="Highlighted Academics"
            description="Feature a specific academic level or achievement with a high-impact layout."
        ...
        >
            <div className="max-w-4xl mx-auto">
                {slots.map((item: any, index: number) => {
                    ...
                    if (!item || item.isSkeleton) {
                        return (
                            <div
                                ...
                                className={`p-10 border-2 border-dashed border-gray-100 rounded-[48px] flex flex-col items-center justify-center gap-6 text-gray-400 min-h-[450px] transition-all duration-500 ${canEditSlot ? "hover:border-red-200 hover:text-[#F54927] hover:bg-red-50/10 cursor-pointer group" : "opacity-60 bg-gray-50/30"}`}
                            >
                                ...
                                <div className="text-center px-8">
                                    <h4 className="text-[20px] font-black tracking-tight text-gray-900 mb-2 leading-tight">Feature Section</h4>
                                    <p className="text-[13px] font-black text-gray-400 uppercase tracking-widest group-hover:text-[#F54927]">
                                        {isEditable ? "Add Featured Content" : (config?.selectionmethod === "manual" ? "Select section to feature" : "No Content Available")}
                                    </p>
                                </div>
                            </div>
                        );
                    ...
                    return (
                        <div key={item.key} className="group relative rounded-[48px] overflow-hidden bg-white border border-gray-100 shadow-sm hover:shadow-2xl transition-all duration-700 flex flex-col md:flex-row min-h-[450px]">
                            {/* Image Section */}
                ...
                <div className="fixed inset-0 z-[300] flex items-center justify-center p-6 bg-gray-900/60 backdrop-blur-md animate-in fade-in duration-300">
                    ...
                    <div className="relative bg-white w-full max-w-5xl rounded-[48px] overflow-hidden shadow-2xl flex flex-col max-h-[85vh] animate-in zoom-in-95 duration-300">
                        <div className="p-10 border-b border-gray-50 flex items-center justify-between bg-white">
                            <div>
                                <h3 className="text-[22px] font-black text-gray-900 tracking-tight">Select Featured Content</h3>
                                <p className="text-[12px] font-bold text-gray-400 uppercase tracking-widest mt-1">Pick the most impactful academic showcase</p>
                            </div>
                        ...
                        <div className="flex-1 overflow-y-auto p-10 no-scrollbar">
                            {academics.length === 0 ? (
                                <div className="py-24 text-center">
                                    ...
                                    <p className="text-gray-900 font-black text-[20px]">No academic records found</p>
                                ...
                                {academics.map((item: any) => (
                                    <button
                                        ...
                                        className={`w-full text-left rounded-[40px] border-2 transition-all p-8 relative flex flex-col gap-4 group ${placements.some((s: any) => s.contentkey === item.key) ? "border-[#F54927] bg-red-50/20" : "border-gray-50 hover:border-red-100 bg-white shadow-sm hover:shadow-xl"}`}
                                    >
                ...
                <div className="fixed inset-0 z-[300] flex items-center justify-center p-6 bg-gray-900/60 backdrop-blur-md animate-in fade-in duration-300">
                    ...
                    <div className="relative bg-white w-full max-w-[1000px] rounded-[48px] shadow-2xl overflow-hidden animate-in zoom-in-95 duration-300 flex flex-col max-h-[95vh]">
                        <div className="p-8 border-b border-gray-50 flex items-center justify-between bg-white">
                            <div className="flex items-center gap-4">
                                <div className="w-12 h-12 bg-[#F54927]/10 text-[#F54927] rounded-2xl flex items-center justify-center">
                                    <Star className="w-6 h-6 fill-[#F54927]" />
                                </div>
                                <div>
                                    <h3 className="text-[22px] font-black text-gray-900 tracking-tight">Featured Academic Editor</h3>
                                    <p className="text-[12px] font-bold text-gray-400 uppercase tracking-widest mt-0.5">Refine your premier curriculum highlight</p>
                                </div>
                            </div>
                                ...
                                <div className="w-full bg-white rounded-[40px] shadow-2xl border border-gray-100 overflow-hidden flex flex-col animate-in slide-in-from-left-6 duration-700">
                                    ...
                                    <div className="p-8 space-y-4">
                                        <div className="space-y-1">
                                            <h4 className="text-[22px] font-black text-gray-900 leading-tight truncate">{editingItem.title || "Section Name"}</h4>
                                            <p className="text-[11px] font-black text-[#F54927] uppercase tracking-widest">{editingItem.subtitle || "Grade Level"}</p>
                                        </div>
                                        <p className="text-[13px] text-gray-500 font-medium leading-relaxed line-clamp-3 italic opacity-80 leading-relaxed whitespace-pre-wrap">
                            ...
                            <div className="flex-1 overflow-y-auto p-12 space-y-12 no-scrollbar">
                                <section className="space-y-8">
                                    <div className="grid grid-cols-2 gap-8">
                                        <div className="space-y-3 col-span-2">
                                            <label className="text-[11px] font-black text-gray-400 uppercase tracking-widest pl-1">Featured Title</label>
                                            <input
                                                type="text"
                                                ...
                                                placeholder="e.g. World-Class Science Labs"
                                            />
                                        </div>
                                        <div className="space-y-3 col-span-2">
                                            <label className="text-[11px] font-black text-gray-400 uppercase tracking-widest pl-1">Subtitle / High-level Detail</label>
                                            <input
                                                type="text"
                                                ...
                                                placeholder="e.g. Empowering Innovation"
                                            />
                                        </div>
                                        <div className="space-y-3 col-span-2">
                                ...
                                {!isIconVariant && (
                                    <section className="space-y-8">
                                        <div className="space-y-3">
                                            <label className="text-[11px] font-black text-gray-400 uppercase tracking-widest pl-1">High-Impact Media</label>
                                            <MediaUpload
                                ...
                                <section className="space-y-8 pb-10">
                                    <div className="flex items-center justify-between">
                                        <label className="text-[11px] font-black text-gray-400 uppercase tracking-widest pl-1">Curriculum Highlights</label>
                                        <button 
                                            type="button"
                        ...
                        <div className="p-10 bg-neutral-50/50 flex items-center justify-between border-t border-gray-100 z-10">
                            {academics.some(f => f.key === editingItem.key) && (
                                <button
