...
export default function LeadershipEditor({ component, schoolKey }: LeadershipEditorProps) {
    const config = component.config as any;
    const isEditable = component.iseditable;
    const tableName = "leadership";
    ...
    const handleSelectRecord = async (recordKey: string) => {
        ...
        try {
            ...
            const response = await upsertComponentData('componentplacement', {
                ...
                componentcode: component.componentcode || 'leadership',
    ...
    const handleSave = async () => {
        ...
        try {
            ...
            console.error("Failed to save leadership member:", err);
        } finally {
            setIsSaving(false);
        }
    };
    ...
    const slots = useMemo(() => {
        ...
        if (config?.selectionmethod === "manual") {
            const limit = itemCount || leadership.length;
            ...
            const limit = itemCount > 0 ? itemCount : leadership.length;
            for (let i = 0; i < limit; i++) {
                result.push(leadership[i] || { isSkeleton: true, displayorder: i + 1 });
            }
        }
        return result;
    }, [leadership, placements, itemCount, config?.selectionmethod, isEditable]);

    return (
        <BaseEditor
            title="Leadership Team"
            description="Manage your school's leadership profiles, messages, and signatures."
            icon={
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-6">
                            ...
                            >
                                ...
                                <div className="text-center px-4">
                                    ...
                                    <p className="text-[11px] font-medium text-gray-400">
                                        {isEditable ? "Click to add leader" : (config?.selectionmethod === "manual" ? "Select leader" : "No Data")}
                                    </p>
                                </div>
                            </div>
                        );
                    ...
                    return (
                        <div key={item.key} className="group relative rounded-[32px] overflow-hidden bg-white border border-gray-100 shadow-sm hover:shadow-xl transition-all duration-300 flex flex-col items-center p-6 pb-8 text-center">
                            ...
                            <div className="flex-1 flex flex-col items-center min-w-0 w-full">
                                ...
                                <p className="text-[10px] font-black text-[#F54927] uppercase tracking-widest mb-3 leading-none">{item.role || "No Role"}</p>
                                <p className="text-[12px] text-gray-400 font-bold mb-3">{item.designation || "No Designation"}</p>
                                <p className="text-[12px] text-gray-500 line-clamp-2 leading-relaxed text-center px-2">{item.message}</p>
                            </div>
                ...
                {isEditable && config?.selectionmethod !== "manual" && (
                    ...
                    >
                        ...
                        <p className="text-[14px] font-black tracking-tight">Add Leader</p>
                    </button>
                )}
            </div>
                ...
                <div className="fixed inset-0 z-[300] flex items-center justify-center p-4 bg-gray-900/40 backdrop-blur-sm shadow-2xl">
                    ...
                    <div className="relative bg-white w-full max-w-2xl rounded-[32px] overflow-hidden shadow-2xl flex flex-col max-h-[80vh]">
                        <div className="p-6 border-b border-gray-50 flex items-center justify-between bg-gray-50/50">
                            <h3 className="text-[18px] font-black text-gray-900 tracking-tight">Select Leader for Slot {pickingForIndex + 1}</h3>
                        ...
                        <div className="flex-1 overflow-y-auto p-6 space-y-3 no-scrollbar">
                            {leadership.length === 0 ? (
                                <div className="py-20 text-center text-gray-400 font-bold">No leadership members found.</div>
                            ) : (
                                leadership.map((item: any) => (
                                    ...
                                    >
                                        ...
                                        <div className="text-left flex-1">
                                            ...
                                            <p className="text-[11px] font-bold text-gray-400 mt-0.5">{item.role} • {item.designation}</p>
                                        </div>
                ...
                <div className="fixed inset-0 z-[300] flex items-center justify-center p-4 bg-gray-900/40 backdrop-blur-sm">
                    ...
                    <div className="relative bg-white w-full max-w-2xl rounded-[32px] overflow-hidden shadow-2xl flex flex-col max-h-[90vh]">
                        <div className="p-6 border-b border-gray-50 bg-gray-50/50 flex items-center justify-between">
                            <h3 className="text-[18px] font-black text-gray-900 tracking-tight">{leadership.some((l: any) => l.key === editingItem.key) ? "Edit Profile" : "New Leader Profile"}</h3>
                        ...
                        <div className="flex-1 overflow-y-auto p-8 space-y-6 no-scrollbar">
                            <div className="grid grid-cols-2 gap-6">
                                ...
                                <div className="space-y-2 col-span-2">
                                    <label className="text-[11px] font-black text-gray-400 uppercase tracking-widest">Signature Image URL</label>
                                    <input
                                        type="text"
                                        value={editingItem.signatureurl || ""}
                                        onChange={e => setEditingItem({ ...editingItem, signatureurl: e.target.value })}
                                        className="w-full px-5 py-4 bg-gray-50 border-2 border-transparent rounded-[20px] focus:bg-white focus:border-red-200 transition-all text-[14px] font-bold outline-none"
                                        placeholder="https://"
                                    />
                                </div>
                                <div className="space-y-2 col-span-2">
                                    ...
                                    <textarea
                                        ...
                                        placeholder="Guiding vision or welcome message..."
                                    />
                                </div>
                            </div>
                        </div>
                        <div className="p-6 bg-gray-50/50 flex items-center justify-between">
                            ...
                            <div className="flex gap-3">
                                ...
                                >
                                    {isSaving ? "Saving..." : (leadership.some((l: any) => l.key === editingItem.key) ? "Update Profile" : "Add Leader")}
                                </button>
                            </div>
                        </div>
                    </div>
