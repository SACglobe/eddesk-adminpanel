...
export default function TestimonialEditor({ component, schoolKey }: TestimonialEditorProps) {
    ...
    const tableName = "testimonialcontent";
    ...
    const handleSave = async () => {
        if (!editingItem.message || !editingItem.authorname) return;
        setIsSaving(true);
        try {
    ...
    const slots = useMemo(() => {
        const result = [];
        const limit = itemCount > 0 ? itemCount : testimonials.length;
        ...
        for (let i = 0; i < limit; i++) {
            if (config?.selectionmethod === "manual") {
                ...
                result.push(testimonials[i] || { isSkeleton: true, displayorder: i + 1 });
            }
        }
        return result;
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-6">
                ...
                {isEditable && config?.selectionmethod !== "manual" && (
                    ...
                    >
                        ...
                        <p className="text-[14px] font-black tracking-tight">Add Testimonial</p>
                    </button>
                )}
            </div>
                    ...
                    <div className="relative bg-white w-full max-w-2xl rounded-[32px] overflow-hidden shadow-2xl flex flex-col max-h-[80vh]">
                        ...
                        <div className="flex-1 overflow-y-auto p-6 space-y-3 no-scrollbar">
                            {testimonials.length === 0 ? (
                                <div className="py-20 text-center text-gray-400 font-bold">No testimonials found.</div>
                            ) : (
                ...
                <div className="fixed inset-0 z-[300] flex items-center justify-center p-4 bg-gray-900/40 backdrop-blur-sm">
                    ...
                    <div className="relative bg-white w-full max-w-2xl rounded-[32px] overflow-hidden shadow-2xl flex flex-col max-h-[90vh]">
                        <div className="p-6 border-b border-gray-50 bg-gray-50/50 flex items-center justify-between">
                            <h3 className="text-[18px] font-black text-gray-900 tracking-tight">{testimonials.some(t => t.key === editingItem.key) ? "Edit Testimonial" : "Add Testimonial"}</h3>
                        ...
                        <div className="p-6 bg-gray-50/50 flex items-center justify-between">
                            ...
                            <div className="flex gap-3">
                                ...
                                <button
                                    disabled={isSaving || !editingItem.message || !editingItem.authorname}
                                    onClick={handleSave}
                                ...
                                >
                                    {isSaving ? "Saving..." : (testimonials.some(t => t.key === editingItem.key) ? "Update Content" : "Add Testimonial")}
                                </button>
                            </div>
                        </div>
                    </div>
